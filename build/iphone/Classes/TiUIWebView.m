...
static NSString *const kBitPay_APIJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_bitpay_apiInjection
{
  ...
  [html appendString:kBitPay_APIJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
  ...
  // attempt to make well-formed HTML and inject in our BitPay_API bridge code
             ...
             baseURL:(NSURL *)baseURL
{
  ...
  content = [[self class] content:content withInjection:[self _bitpay_apiInjection]];
            ...
            mimeType:(NSString *)mimeType
{
  ...
  [request setValue:[NSString stringWithFormat:@"%lu", (localId++)] forHTTPHeaderField:@"X-BitPay_API-Local-Id"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
  ...
  if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
    ...
    if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
      [LocalProtocolHandler setContentInjection:[self _bitpay_apiInjection]];
    }
