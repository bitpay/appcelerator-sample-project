...
NSString *BitPay_API$ModuleRequireFormat = @"(function(exports){"
...
@implementation BitPay_APIObject
...
{
  // create BitPay_API global object
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  // Load the "BitPay_API" object into the global scope
  NSString *basePath = (url == nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
  _bitpay_api = [[BitPay_APIObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

  TiContextRef jsContext = [kroll context];
  TiValueRef tiRef = [KrollObject toValue:kroll value:_bitpay_api];

  NSString *_bitpay_apiNS = [NSString stringWithFormat:@"T%sanium", "it"];
  TiStringRef prop = TiStringCreateWithCFString((CFStringRef)_bitpay_apiNS);
  ...
  // For other files, it will be injected via the `BitPay_API$ModuleRequireFormat` property
  ...
  if (preload != nil) {
    for (NSString *name in preload) {
      KrollObject *ti = (KrollObject *)[_bitpay_api valueForKey:name];
...
- (KrollWrapper *)loadCommonJSModule:(NSString *)code withSourceURL:(NSURL *)sourceURL
{
  ...
  NSString *js = [[NSString alloc] initWithFormat:BitPay_API$ModuleRequireFormat, dirname, filename, code];
...
- (KrollWrapper *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
  ...
  if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
    @throw [NSException exceptionWithName:@"org.bitpay_api.kroll"
