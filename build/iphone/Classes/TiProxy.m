...
//Rare exceptions to indicate a bug in the _bitpay_api code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString *)qualifiedName withProperties:(NSDictionary *)properties inContext:(id<TiEvaluator>)context
{
  ...
  if (proxyClass == nil) {
    NSString *_bitpay_api = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium."];
    if ([qualifiedName hasPrefix:_bitpay_api]) {
    ...
    if (proxyClass == nil) {
      ...
      @throw [NSException exceptionWithName:@"org.bitpay_api.module"
